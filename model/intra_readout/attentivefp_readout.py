...
class GlobalPool(nn.Module):
    ...
    def __init__(self, feat_size, dropout):
        ...
        self.compute_logits = nn.Sequential(
            nn.Linear(2 * feat_size, 1), #.to("cuda:0"),
            nn.LeakyReLU()#.to("cuda:0")
        )
        self.project_nodes = nn.Sequential(
            nn.Dropout(dropout), # .to("cuda:0"),
            nn.Linear(feat_size, feat_size) # .to("cuda:0")
        )
        self.gru = nn.GRUCell(feat_size, feat_size) # .to("cuda:0")
